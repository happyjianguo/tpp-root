package com.fxbank.cap.paf.nettty;
	...
	protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
		...
		if (login.equals("0")) { // 0-业务类交易，服务端只会收到业务交易
			...
			try (Jedis jedis = myJedis.connect()) {
				String encryptSessionKey = jedis.get(PAF.SESSIONKEY);
			...
			bodyBuf.append(new String(decryptMsg, PAF.ENCODING));
		} else if (login.equals("1")) {
			logUtil.error(logger, "测试用签到交易");
			bodyBuf.append(new String(data, PAF.ENCODING));
		} else {
