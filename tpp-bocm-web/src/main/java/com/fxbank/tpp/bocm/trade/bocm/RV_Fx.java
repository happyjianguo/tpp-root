...
public class RV_Fx implements TradeExecutionStrategy {
	...
	public DataTransObject execute(DataTransObject dto) throws SysTradeExecuteException {
		...
		if(model==null){
			...
			if(model.getTxDate()!=dto.getSysDate()) {
				myLog.error(logger, "不能隔日冲正，交易日期" + model.getTxDate() + 
						"渠道流水号" + model.getPlatTrace()+",当前日期"+dto.getSysDate());
	...
	*/
	public ESB_REP_30014000101 hostReversal(DataTransObject dto,BocmRcvTraceQueryModel model)
		...
		reqBody_30014000101.setChannelSeqNo(ESB.SYSTEM_ID+model.getPlatDate()+String.format("%08d",model.getPlatTrace()));
