...
import com.fxbank.tpp.bocm.dto.bocm.ChkFile;
...
//		*/
		...
		*/	
				...
				int rcvTotal = Integer.parseInt(rcvCheckFlag1)+Integer.parseInt(rcvCheckFlag2)+Integer.parseInt(rcvCheckFlag3)+Integer.parseInt(rcvCheckFlag4);
				...
				int sndTotal = Integer.parseInt(sndCheckFlag1)+Integer.parseInt(sndCheckFlag2)+Integer.parseInt(sndCheckFlag3)+Integer.parseInt(sndCheckFlag4);
				
				int tolCnt = rcvTotal + sndTotal;
				
				String rcvTotalNum = rcvTraceService.getRcvTotalChkSum(myLog, date);
				if(rcvTotalNum==null){
					rcvTotalNum = "0.00";
				}
				...
				for(BocmRcvTraceQueryModel model :upRcvTraceList){
					//本方交易流水号
					ChkFile chk = modelToChkFile(model);
					String chkdata = chk.creaFixPack();
					myLog.info(logger, "转换对账交易明细报文："+chkdata);
					fileTxt.append(chkdata);					
				}			
				
				myLog.info(logger, "返回报文文件长度："+fileTxt.length());
				rep.setFilLen(fileTxt.length()+"");	
				rep.setTolCnt(tolCnt+"");
//				rep.setTolAmt(new BigDecimal(rcvTotalNum));
//				if(fileTxt.length()==0){
//					rep.setFilTxt(null);
//					BocmTradeExecuteException e = new BocmTradeExecuteException(BocmTradeExecuteException.BOCM_E_10013,"无交易记录");
//					throw e;
//				}else{
//					rep.setFilTxt(fileTxt.toString());
//				}
...
//				myLog.info(logger, "返回报文体："+rep.creaFixPack());
	...
	private ChkFile modelToChkFile(BocmRcvTraceQueryModel model){
		ChkFile chk = new ChkFile();
		//给交行返回交易流水
...
//		chk.setLogNo(model.getBocmTraceno());
		...
		chk.setTxnAmt(model.getTxAmt());
		//手续费收取方式
		chk.setFeeFlg(model.getFeeFlag());
		//开户行手续费
		chk.setFee(model.getFee());
		//通存通兑业务模式 0 现金 1 转账
