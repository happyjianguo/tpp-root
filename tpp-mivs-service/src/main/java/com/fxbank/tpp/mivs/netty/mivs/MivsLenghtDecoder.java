 ...
 */
public class MivsLenghtDecoder<T> extends ByteToMessageDecoder {
	...
	public MivsLenghtDecoder(MyLog myLog, NettySyncSlot<T> slot) {
		this.myLog = myLog;
		this.slot = slot;
	}
	...
	protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
		...
		if ( readableLen < DATALENGTH ) {
			...
			this.myLog.error(logger, "接收数据太短，不处理["+readableLen+"]["+new String(lenBytes, MivsClient.CODING)+"]");
			return null;
		}
		in.markReaderIndex();
		...
		String lenStr = new String(lenbyte,MivsClient.CODING);
		if (!isInteger(lenStr)) {
		...
		if (len > onceLen) {
			while (in.readableBytes() > 0) {
				...
				buf.readBytes(data);
				msgbuf.append(new String(data, MivsClient.CODING));
			...
			buf.readBytes(req);
			msgbuf.append(new String(req, MivsClient.CODING));
			ReferenceCountUtil.release(buf);
		}
