...
bocm_repcd NVARCHAR2(20) NULL ,
bocm_repmsg NVARCHAR2(200) NULL ,
...
;
COMMENT ON TABLE bocm_snd_log IS '交通银行柜面通往账流水日志';
...
COMMENT ON COLUMN bocm_snd_log.fee IS '开户行手续费';
COMMENT ON COLUMN bocm_snd_log.proxy_flag IS '代理手续费收取标志 规则：交易受理方收取代理手续费0 （交行支付代理手续费）1 （交行收取代理手续费）';
COMMENT ON COLUMN bocm_snd_log.proxy_fee IS '代理手续费';
COMMENT ON COLUMN bocm_snd_log.bocm_fee_flag IS '手续费对账文件，手续费收取方式 0账户内扣 1现金外收';
COMMENT ON COLUMN bocm_snd_log.bocm_fee IS '交行获取的手续费记录';
